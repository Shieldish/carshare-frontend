...
export default function AddVehicleForm() {
  ...
  const validateImageFile = useCallback((file: File): string | null => {
    if (!ACCEPTED_IMAGE_TYPES.includes(file.type)) return `Format non accepté.`;
    if (file.size > MAX_FILE_SIZE) return `Trop volumineux (max 5MB).`;
    return null;
  }, []);
  ...
  }, [imageFiles.length, validateImageFile]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!locationGps) { setError("Veuillez cliquer sur la carte."); setIsLoading(false); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    
    if (!registrationFile || !insuranceFile || !technicalControlFile) {
      setError("La Carte Rose, l'Assurance ET le Contrôle Technique sont obligatoires.");
      setIsLoading(false); return;
    }

    if (!insuranceConfirmed) {
      setError("Vous devez confirmer avoir déclaré cet usage à votre assureur.");
      setIsLoading(false); return;
    }
    ...
    const vehicleData: VehicleData = {
      ...
      // ✅ NOUVEAU : Récupération des dates
      insuranceExpiryDate: formData.get('insuranceExpiryDate') as string,
      technicalControlExpiryDate: formData.get('technicalControlExpiryDate') as string,
    };
    ...
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Marque *</label><input type="text" name="make" required className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
          <div><label className="block text-sm font-medium mb-1">Modèle *</label><input type="text" name="model" required className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Année *</label><input type="number" name="manufacturingYear" required min="1900" max={new Date().getFullYear() + 1} className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
          <div><label className="block text-sm font-medium mb-1">Kilométrage (km)</label><input type="number" name="mileage" min="0" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
        </div>

        <div className="border-t pt-6">
          <h3 className="text-lg font-medium mb-4">Caractéristiques techniques</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div><label className="block text-sm font-medium mb-1">Type</label><select name="type" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"><option value="">Sélectionnez</option>{VEHICLE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1">Carburant</label><select name="fuelType" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"><option value="">Sélectionnez</option>{FUEL_TYPES.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">Transmission</label><select name="transmission" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"><option value="">Sélectionnez</option>{TRANSMISSION_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1">Sièges</label><input type="number" name="seats" min="1" max="50" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
          </div>
          <div className="mt-4">
            <label className="block text-sm font-medium mb-1">Équipements</label>
            <input type="text" name="equipment" value={equipmentText} onChange={(e) => setEquipmentText(e.target.value)} maxLength={MAX_EQUIPMENT_LENGTH} className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" placeholder="Ex: Climatisation, GPS..." />
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">Prix par jour (FBu) *</label><input type="number" name="ratePerDay" required min="0" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
            <div><label className="block text-sm font-medium mb-1">Prix par semaine (FBu)</label><input type="number" name="ratePerWeek" min="0" className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
          </div>
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
            <div className="flex items-start">
              ...
              <div className="ml-3 text-sm"><label htmlFor="supportsDriver" className="font-medium cursor-pointer">Inclus un chauffeur ?</label><p className="text-gray-500">Cochez si le prix inclut la prestation d&apos;un chauffeur.</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">Province</label><select name="provinceId" value={selectedProvince} onChange={handleProvinceChange} disabled={isLoadingProvinces} className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"><option value="">{isLoadingProvinces ? "Chargement..." : "-- Non spécifié --"}</option>{provinces.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1">Commune</label><select id="communeId" name="communeId" disabled={!selectedProvince} className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"><option value="">{isLoadingCommunes ? "Chargement..." : "-- Non spécifié --"}</option>{communes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>
          <div className="mt-4 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
            <label className="block text-sm font-medium mb-1">Adresse précise</label>
            <div className="flex space-x-2"><input type="text" name="address" value={addressText} onChange={(e) => setAddressText(e.target.value)} className="flex-1 px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /><button type="button" onClick={handleSearchLocationOnMap} disabled={isSearchingMap} className="bg-blue-100 text-blue-700 px-4 py-2 rounded-md font-semibold">{isSearchingMap ? 'Recherche...' : 'Centrer'}</button></div>
          </div>
          <div className="mt-6" ref={mapRef}><label className="block text-sm font-medium mb-1">Emplacement *</label><LocationPickerMap onLocationSelect={(lat, lng) => setLocationGps(`${lat}, ${lng}`)} centerPosition={mapCenter} forceMarkerPosition={locationGps ? [parseFloat(locationGps.split(',')[0]), parseFloat(locationGps.split(',')[1])] : null} /><input type="hidden" name="locationGps" value={locationGps} /></div>
        </div>

        <div className="border-t pt-6">
          <label className="block text-sm font-medium mb-1">Description</label>
          <textarea name="description" rows={4} value={descriptionText} onChange={(e) => setDescriptionText(e.target.value)} maxLength={MAX_DESCRIPTION_LENGTH} className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" />
        </div>

        <div className="border-t pt-6">
          <h3 className="text-lg font-medium mb-4 flex items-center gap-2">Documents Légaux <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full">Obligatoires</span></h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
              <label className="block text-sm font-medium mb-2">Carte Rose *</label>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
              ...
              <div className="border-t dark:border-gray-700 pt-2 mt-2">
                <label className="block text-xs font-semibold text-blue-600 dark:text-blue-400 mb-1">Expire le *</label>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
              <label className="block text-sm font-medium mb-2">Contrôle Tech. *</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {imagePreviews.map((p, i) => (
                <div key={i} className="relative group w-full h-24">
                  <Image src={p} alt={`Aperçu`} fill className="object-cover rounded-lg border shadow-sm" unoptimized />
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800 mb-6">
          <div className="flex items-start space-x-3">
            ...
            <label htmlFor="insurance-check" className="text-sm text-gray-700 dark:text-gray-300 leading-tight">
              Je certifie sur l&apos;honneur avoir informé mon assureur que ce véhicule est utilisé pour de la location entre particuliers sur la plateforme BudaxDrive. Je reconnais que BudaxDrive n&apos;est qu&apos;un tiers de confiance et décline toute responsabilité en cas de fausse déclaration.
            </label>
          </div>
        </div>
        ...
        <div className="pt-6">
          <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors font-medium">
            {isLoading ? 'Ajout en cours...' : 'Soumettre le véhicule pour validation'}
          </button>
        </div>
      </form>
    </div>
