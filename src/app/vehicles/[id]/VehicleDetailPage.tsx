...
interface VehicleDetailPageProps {
  // Véhicule déjà récupéré côté serveur (page.tsx), réutilisé pour le rendu
  // initial afin d'éviter un aller-retour réseau redondant + le flash de
  // chargement. `null`/`undefined` si le fetch serveur a échoué (404, etc.) —
  // dans ce cas on retombe sur le fetch client existant ci-dessous.
  initialVehicle?: Vehicle | null;
  vehicleId?: string;
}

export default function VehicleDetailPage({ initialVehicle = null, vehicleId: vehicleIdProp }: VehicleDetailPageProps = {}) {
  const t = useTranslations('vehicles.detail');
  const tReviews = useTranslations('vehicles.reviews');
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const vehicleId = vehicleIdProp ?? (params.id as string);
  const { user } = useAuth();
  const [vehicle, setVehicle] = useState<Vehicle | null>(initialVehicle);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(!initialVehicle);
  ...
  useEffect(() => {
    if (initialVehicle) {
      // Le serveur (page.tsx) a déjà fourni le véhicule : on évite de le
      // re-fetcher, mais on charge quand même le résumé des avis côté client.
    ...
    // Filet de sécurité : si le fetch serveur a échoué (ex: 404, réseau), on
    // retente ici côté client comme avant.
  ...
  const getLocationDisplay = (vehicle: Vehicle): string => {
    ...
    // Sur la page détail, on affiche toute la hiérarchie : quartier, commune, province.
    if (vehicle.commune) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-xl p-6 shadow-lg border border-border">
              ...
              {/* Loué par (lien vers le profil hôte public) */}
              {vehicle.ownerId && (
                <Link
                  href={`/hosts/${vehicle.ownerId}`}
                  className="flex items-center text-foreground mb-4 bg-muted hover:bg-muted/70 px-4 py-3 rounded-lg transition-colors group"
                >
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0 mr-3">
                    {vehicle.ownerProfilePictureUrl ? (
                      <Image
                        src={vehicle.ownerProfilePictureUrl}
                        alt=""
                        width={40}
                        height={40}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-muted-foreground">{t('rentedBy')}</p>
                    <p className="font-semibold text-primary group-hover:underline truncate">
                      {vehicle.ownerFirstName} {vehicle.ownerLastInitial}
                    </p>
                  </div>
